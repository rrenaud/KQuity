...
the fixed-point reference used "int features + int weights ->
...
the pure-int pipeline reproduces the float-after-dequantize
...
returns the recovered float logit. The testbench requires
HLS == kquity_logit_q8 for every event in the test pool.

The bit-exact path differs from the float path by O(2^-15) and is a
...
# --- Verification: int ref vs float-after-dequantize -------------
...
# --- CLI: load weights, emit constants header ---------------------
...
def main():
    ...
    lines.append("// Generated by tests/kquity_pressure_ref.py")
